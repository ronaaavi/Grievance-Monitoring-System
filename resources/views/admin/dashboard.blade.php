...
@section('sidebar')
    ...
    <a href="{{ route('admin.grievancereports') }}" 
       class="flex items-center space-x-4 px-4 py-4 rounded-lg font-semibold 
              {{ request()->routeIs('admin.grievancereports') ? 'bg-red-800 text-white' : 'text-gray-700 hover:bg-red-800 hover:text-white' }}">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" 
             viewBox="0 0 24 24" class="w-6 h-6">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
...
@section('content')
    ...
    <div class="bg-[#6f0909] text-white rounded-xl flex items-center justify-between pl-10 pt-6 mb-10 flex-shrink-0">
        <div class="flex flex-col justify-between h-40">
            <p class="text-medium text-gray-200">{{ now()->format('F j, Y') }}</p>
            <div>
        ...
        <img src="/images/Sticker.png" alt="Admin Illustration" class="h-50 object-contain">  
    </div>
